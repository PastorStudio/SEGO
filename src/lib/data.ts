...
export async function getTickets(): Promise<Ticket[]> {
  ...
  const rows = (data as any[]) || [];
  return rows.map((t) => ({ ...t, comments: JSON.parse(t.comments || '[]') })) as Ticket[];
}
...
export async function getUnreadMessageCount(currentUserId: string): Promise<Record<string, number>> {
  ...
  for (const message of (data as { senderId: string }[]) || []) {
...
export async function addProject(project: Omit<Project, 'id'>, userName: string): Promise<Project> {
  ...
  await addNotification({ message: `${userName} creó el nuevo proyecto: "${(data as any).name}"`, link: `/projects/${(data as any).id}` });
...
export async function addUser(
  ...
  await addNotification({ message: `Un nuevo usuario se ha unido: ${(data as any).name} (${(data as any).role})`, link: '/users' });
...
export async function setUserStatus(userId: string, status: 'online' | 'offline'): Promise<void> {
  ...
  if (status === 'offline') updateData.lastSeen = new Date().toISOString();
...
export async function addClient(client: Omit<Client, 'id'>): Promise<Client> {
  ...
  await addNotification({ message: `Nuevo cliente registrado: ${(data as any).name}`, link: '/clients' });
...
export async function addInvoice(invoice: Omit<Invoice, 'id' | 'clientName'>, userName: string): Promise<Invoice> {
  ...
  await addNotification({
    message: `Se ha creado una nueva factura (${(data as any).id}) para ${(data as any).clientName}`,
    link: `/invoices/${(data as any).id}`,
  });
...
export async function updateInvoice(updatedInvoice: Invoice, userName: string): Promise<void> {
  ...
  await addNotification({
    message: `La factura "${updatedInvoice.id}" ha sido actualizada.`,
...
export async function addTask(task: Omit<Task, 'id'>, userName: string): Promise<Task> {
  ...
  await addNotification({ message: `${userName} agregó la tarea "${(data as any).title}"`, link: `/projects/${(data as any).projectId}` });
  revalidatePath('/tasks');
  return data as Task;
}

export async function addImmediateTask(
  task: Omit<ImmediateTask, 'id' | 'comments'>,
  assignerName: string
): Promise<ImmediateTask> {
  const newTaskData: any = { ...task, id: `itask-${Date.now()}`, comments: JSON.stringify([]) };
...
export async function addWarehouseRequest(
  ...
  await addNotification({
    message: `${userName} creó una solicitud de almacén para ${project?.name || ''}`,
    link: `/warehouse`,
  });
...
export async function updateWarehouseRequest(
  requestId: string,
  newStatus: WarehouseRequest['status'],
  userName: string
): Promise<void> {
...
async function addComment(
  ...
  const comments = JSON.parse((entity as any).comments || '[]');
...
export async function addTicket(ticketData: Omit<Ticket, 'id' | 'createdAt' | 'comments'>): Promise<Ticket> {
  ...
  await addNotification({ message: `Nuevo ticket creado: "${(data as any).title}"`, link: `/tickets/${(data as any).id}` });
...
export async function updateTicket(updatedTicket: Ticket): Promise<void> {
  const updateData: any = { ...updatedTicket, comments: JSON.stringify(updatedTicket.comments || []) };
  ...
  await addNotification({
    message: `El ticket "${updatedTicket.title}" ha sido actualizado.`,
